...
export class ZapExecutor {
  ...
  openRouterService;
  telegramService;
  constructor(supabase, gmailService, notionService, emailParser, logger){
  ...
  async executeActiveZaps(userId) {
    this.logger.info('Fetching active zaps', {
      userId
    });
    let query = this.supabase.from('zaps').select('id, user_id, name').eq('is_active', true);
    if (userId) {
    ...
    const results = await Promise.allSettled(zaps.map((zap)=>this.executeSingleZap(zap.id)));
  ...
  async executeSingleZap(zapId) {
    ...
    try {
      ...
      const { data: zap, error } = await this.supabase.from('zaps').select(`
          *,
          steps:zap_steps(
            *
          )
      ...
      // Sort steps by order
      const sortedSteps = (zap.steps || []).sort((a, b)=>a.step_order - b.step_order);
      ...
      const triggerStep = sortedSteps.find((step)=>step.step_type === 'trigger');
      if (!triggerStep) {
        return {
          zapId,
          success: false,
      ...
      const actionSteps = sortedSteps.filter((step)=>step.step_type === 'action');
  ...
  async executeTriggerStep(step, userId) {
    const { service_name, event_type, configuration } = step;
    if (service_name === 'gmail' && event_type === 'new_email') {
      const filters = {
        keywords: configuration.keywords?.split(',').map((k)=>k.trim()).filter(Boolean),
  ...
  async executeActionStep(step, userId, email) {
    const { service_name, event_type, configuration } = step;
    ...
    if (service_name === 'openrouter' && event_type === 'process_with_ai') {
      ...
      const processedEmail = await this.openRouterService.processEmailWithAI(email, config);
  ...
  async updateZapStats(zapId, emailsProcessed) {
    try {
      const { error } = await this.supabase.rpc('update_zap_run_stats', {
        zap_id_param: zapId,
  ...
  async isEmailAlreadyProcessed(emailId, zapId) {
    try {
      this.logger.info(`Checking if email ${emailId} is already processed for zap ${zapId}`);
      // First, let's verify the table exists and we can access it
      const { data: tableData, error: tableError } = await this.supabase.from('processed_emails').select('id', {
        count: 'exact'
      }).limit(1);
      this.logger.info(`Table access test:`, {
        tableData,
        tableError
      });
      if (tableError) {
        this.logger.error('Cannot access processed_emails table:', tableError);
        return false; // On table access error, process the email to be safe
      }
      const { data, error } = await this.supabase.from('processed_emails').select('id, processed_at, user_id').eq('email_id', emailId).eq('zap_id', zapId).single();
      this.logger.info(`Processed emails query result:`, {
        data,
        error,
      ...
      if (data) {
        this.logger.info(`Email ${emailId} already processed on ${data.processed_at} by user ${data.user_id}`);
        return true;
      }
      this.logger.info(`Email ${emailId} not found in processed_emails, will process`);
      return false;
    } catch (error) {
      this.logger.error('Error checking if email is processed:', error);
      return false; // On error, process the email to be safe
  ...
  async markEmailAsProcessed(emailId, zapId, userId, emailSubject, emailSender) {
    try {
      this.logger.info(`Attempting to mark email ${emailId} as processed for zap ${zapId} by user ${userId}`);
      const insertData = {
        email_id: emailId,
        zap_id: zapId,
        user_id: userId,
      ...
      const { data, error } = await this.supabase.from('processed_emails').insert(insertData).select('*');
      this.logger.info(`Insert result:`, {
        data,
        error
      });
      if (error) {
        ...
        if (error.code === '23505') {
          this.logger.info(`Email ${emailId} was already marked as processed (unique constraint violation)`);
        }
      // Don't throw error here, as we still want the zap to succeed
      } else {
        this.logger.info(`Successfully marked email ${emailId} as processed for zap ${zapId}. Inserted record:`, data);
      }
    } catch (error) {
      this.logger.error('Exception when marking email as processed:', error);
    // Don't throw error here, as we still want the zap to succeed
    }
  }
  // Simple in-memory execution lock using a Set to track running zaps
  static runningZaps = new Set();
  async acquireExecutionLock(lockId) {
  ...
  async releaseExecutionLock(lockId) {
   ...
   */ processStepConfiguration(configuration, email, stepResults) {
    const processed = {
      ...configuration
    };
   ...
   */ processTemplate(template, variables, stepResults) {
