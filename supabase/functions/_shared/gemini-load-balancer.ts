  ...
  cleanup() {
    ...
    for (const [sessionId, context] of this.conversationContexts.entries()) {
      if (context.messages.length > 0) {
        // Check if last message is older than maxAge
        // Note: This is simplified - in production you'd want timestamps on messages
        if (this.conversationContexts.size > 1000) { // Only clean if we have too many
          this.conversationContexts.delete(sessionId);
        }
      }
    }
...
export const geminiLoadBalancer = new GeminiLoadBalancer();
